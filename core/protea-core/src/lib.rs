...
//!
//! This crate deliberately starts with standard-library-only types.
//! Persistence, IPC and network synchronization are separate engineering
//! concerns and will be added behind stable interfaces.
...
impl PermissionSet {
    ...
    pub fn revoke(&mut self, permission: Permission) {
        self.granted.retain(|item| *item != permission);
    }
...
impl ProteaState {
    pub fn new(device: DeviceProfile) -> Self {
        Self {
            ...
            device,
            mode: ProteaMode::Office,
        }
    }

    pub fn set_identity(&mut self, identity: ProteaIdentity) {
        self.identity = Some(identity);
    }

    pub fn set_mode(&mut self, mode: ProteaMode) {
        self.mode = mode;
    }
}
...
mod tests {
    ...
    fn state_starts_in_office_mode() {
        ...
        let state = ProteaState::new(device);
